...
exports.FT_RECEIVE_RF_DATA = 0x81;      // RX received 16bit address!
...
function packetToJS(packet) {
  ...
  if (packet[0]== exports.FT_AT_RESPONSE) {
    ...
    p = {
      type: 'RF Data',
      remote16: {dec: packet.slice(1,3), hex: byteArrayToHexString(packet.slice(1,3))},
      RSSI: packet[3],
      options: packet[4],
      raw_data: packet.slice(5),
      data: "",
      bytes: packet,
      values: []
    }
    ...
    for(var i = 0; i < p.raw_data.length; i = i+3){
        p.values.push( p.raw_data[i] << 16 | p.raw_data[i + 1] << 8 | p.raw_data[i + 2] );
    }
    for(i in p.raw_data) {
